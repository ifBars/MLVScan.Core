        ...
        private string? ExtractInvokedMethodName(Mono.Collections.Generic.Collection<Instruction> instructions, int currentIndex)
        {
            ...
            for (int i = Math.Max(0, currentIndex - 20); i < currentIndex; i++)
            {
                    ...
                    instr.OpCode == OpCodes.Stloc_3 || instr.OpCode == OpCodes.Stloc_S)
                {
                    ...
                    if (methodNameFromLocal != null && i > 0 && instructions[i - 1].OpCode == OpCodes.Ldstr)
                    {
                        if (instr.OpCode == OpCodes.Stloc_0)
                            ...
                            localVarIndex = 3;
                        else if (instr.Operand is VariableDefinition varDef)
                ...
                if (localVarIndex >= 0 && methodNameFromLocal != null)
                {
                    ...
                    if (instr.OpCode == OpCodes.Ldloc_0 && localVarIndex == 0)
                        isLoadingTrackedLocal = true;
                    else if (instr.OpCode == OpCodes.Ldloc_1 && localVarIndex == 1)
                        isLoadingTrackedLocal = true;
                    else if (instr.OpCode == OpCodes.Ldloc_2 && localVarIndex == 2)
                        isLoadingTrackedLocal = true;
                    else if (instr.OpCode == OpCodes.Ldloc_3 && localVarIndex == 3)
                        isLoadingTrackedLocal = true;
                    else if (instr.Operand is VariableDefinition varDef2 && varDef2.Index == localVarIndex)
                        isLoadingTrackedLocal = true;
